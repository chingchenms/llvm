//===- CoroPassManager.cpp - Manager for Coroutine Passes -----------------===//
//
...
struct CoroPassManager : public ModulePass, CoroutineInfo {
  static char ID; // Pass identification, replacement for typeid
  CoroPassManager() : ModulePass(ID) {}
...
#endif
#if 0
  struct CoroPassManager : public ModulePass, CoroutineCommon {
    static char ID; // Pass identification, replacement for typeid
    CoroPassManager() : ModulePass(ID) {}
...
#endif
#if 0
  class CoroPassManager : public Inliner {
  public:
    CoroPassManager() : Inliner(ID) {
...
char CoroPassManager::ID = 0;
// static RegisterPass<CoroPassManager> cpm("CoroPassManager", "coroutine pass
// manager");
namespace llvm {
INITIALIZE_PASS_BEGIN(CoroPassManager, "coro-all", "coroutine pass manager",
                      false, false)
INITIALIZE_PASS_DEPENDENCY(AssumptionCacheTracker)
INITIALIZE_PASS_DEPENDENCY(CallGraphWrapperPass)
INITIALIZE_PASS_DEPENDENCY(TargetLibraryInfoWrapperPass)
INITIALIZE_PASS_END(CoroPassManager, "coro-all", "coroutine pass manager",
                    false, false)
